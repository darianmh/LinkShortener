...
;(function ($) {
	...
	$.SOW.helper = {
		...
		cssLocation: function() {
			...
			var currentScript;
			var scripts = jQuery('head link[rel=stylesheet]');
...
;(function ($) {
	...
	$.SOW.core.gdpr = {
		...
		process: function(_selector) {
			...
			var __gdpr = Cookies.get($.SOW.core.gdpr.config.cookie_name, { path: '/' });
			...
			if(__gdpr == null) {
				...
				Cookies.set($.SOW.core.gdpr.config.cookie_name, __gdpr, { expires: $.SOW.core.gdpr.config.cookie_expire, path: '/' }); // refresh
			...
			jQuery(' a.btn-cookie-accept:not(.js-gdprified)').addClass('js-gdprified').on('click', function(e) {
				...
				/* 
					values:
						...
						2 = website + advertising
				*/
				Cookies.set($.SOW.core.gdpr.config.cookie_name, 0, { expires: $.SOW.core.gdpr.config.cookie_expire, path: '/' });
				jQuery(_selector).remove(); // just in case
		...
		*/
		extend: function(__gdprLevel) {
			...
			Cookies.set($.SOW.core.gdpr.config.cookie_name, __gdprLevel, { expires: $.SOW.core.gdpr.config.cookie_expire, path: '/' });
			return true;
		},
